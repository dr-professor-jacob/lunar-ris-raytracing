...
**The Problem:**
NASA's Artemis III mission plans to land at the Malapert Massif on the Moon's south pole [[nasa.gov]](https://www.nasa.gov/news-release/nasa-identifies-candidate-regions-for-landing-next-americans-on-moon/). While the landing zone has a great direct line-of-sight to Earth for communications, the steep valleys right next to it are stuck in massive "radio shadows" where astronauts and rovers would lose signal.

**The Solution:**
We propose putting a 100-meter deployable tower (like Honeybee Robotics' LUNARSABER, selected for DARPA's LunA-10 study [[darpa.mil]](https://www.darpa.mil/news-events/2023-12-05)) on the mountain peak. By strapping a smart reflector (an RIS) to the top, we can "bounce" the radio signals from the landing site down into the dark valleys, keeping everyone connected.

We will test this in a physics simulation using **NVIDIA's Sionna-RT** raytracer [[nvidia.com]](https://developer.nvidia.com/sionna) to map out exactly how the radio waves will bounce across a real 3D model of the mountain. We will simulate the Starship Lander (the base station) and the RIS mast to calculate exact signal strengths across the grid, and build an interactive 3D dashboard to visually demonstrate the coverage difference.

---

## 2. Simulation Logic (Pseudocode)

This section explains what the simulation code does in plain English. Think of it as a playbook for the Python script.

### Step 1: Setup the World
- **Load the Map**: Load the 3D model of the Malapert Massif mountain (`nasa_malapert_dem.obj`).
- **Create Rovers**: Create an 80x80 grid of "Rovers" (Receivers) and drop them straight down onto the mountain surface so they follow the terrain.
- **Set Physics**: Load the radio materials (Lunar Regolith, matching standard dielectric properties $\epsilon_r \approx 3.1$ [[mdpi.com]](https://www.mdpi.com/2072-4292/14/19/4890)) so the physics engine knows how radio waves will bounce off the dirt.

### Step 2: Place the Transmitters
- **Base Station**: Place the Base Station (Starship HLS) on the flat Earth-facing slope, 50 meters up in the air (the approximate height of the SpaceX HLS [[nasa.gov]](https://www.nasa.gov/humans-in-space/human-landing-system/)).
- **RIS Relay**: Place the Reconfigurable Intelligent Surface (RIS) Relay on the very peak of the mountain, 100 meters up in the air so it can "see" over the rocky shoulder.

### Step 3: Run the Raytracing Physics
For each frequency (3.5 GHz, 10 GHz, and 28 GHz), do the following heavy lifting:
- **Shoot Rays from Base**: Calculate the signal strength (dBm) for all 6,400 Rovers based on direct line-of-sight and bounces from the Base Station.
- **Shoot Rays from RIS**: Calculate the signal strength (dBm) for all 6,400 Rovers based on the RIS Relay.
- **Combine**: The rover's final signal strength is simply the maximum of the two (the rover connects to whichever signal is stronger).

### Step 4: Export and Visualize
- **Save Data**: Save all the raw coordinates and signal strengths to a database (`lunar_telemetry.db`) and a file (`malapert_coverage_data.npz`).
- **Build 3D Dashboard**: Use Plotly to draw the final interactive 3D map:
  - Draw the mountain terrain.
  - Draw the Base Station and RIS Mast as 3D cylinders.
  - Draw the rovers as dots colored by their signal strength (red = good, blue = bad).
  - Add a button to toggle between the Baseline (Starship only) and RIS-assisted coverage so you can see the difference the RIS makes!

---

## 3. Data Pipeline Audit

This section traces the complete lifecycle of the data, showing exactly how raw simulation variables are transformed into the interactive 3D visual dashboard.

### Environment & Inputs
Before any RF physics are calculated, the physical environment is structured:
* **Receiver Grid:** An 80x80 mathematical grid is created. Rays are cast straight down (Z-axis) onto the `nasa_malapert_dem.obj` terrain mesh to find the exact surface height for all 6,400 Rovers (`rx_positions`).
* **Transmitters:** 
  * `bs_pos`: The Starship Base Station is anchored at `X=-500` and elevated by 50m.
  * `ris_pos`: The RIS Relay is anchored at the peak (`X=0`) and elevated by 100m to clear the convex terrain.

### Physics & Propagation
For each frequency (e.g., 28 GHz), Sionna-RT's `PathSolver` shoots rays from the transmitters to all 6,400 receivers. 

* **Coefficient Extraction:** The path solver returns real and imaginary coefficients (`a_list`).
* **Power Calculation:** The raw received power (`p_bs` and `p_ris`) is calculated using the sum of squares across the complex coefficients.
* **dBm Conversion & Logic:**
  * `power_bs`: The raw baseline power from the Base Station, converted to decibels (dBm).
  * `power_combined`: The script calculates this as `np.maximum(power_bs, power_ris)`. This assumes the rover will automatically connect to whichever signal is stronger.

### Data Storage
Once the physics are calculated, the data is dumped to disk to decouple the heavy computation from the visualization logic.
* **Visualization Output:** `rx_positions`, `bs_pos`, `ris_pos`, and all frequency power arrays are compressed into a NumPy archive at `results/malapert_coverage_data.npz`.
* **Database Output:** It is also exported to an SQLite database (`lunar_telemetry.db`).

### 3D Visual Dashboard
The `build_dashboard()` function loads the `.npz` file and maps the raw data directly to 3D visual elements in `results/dashboard.html`.

* **Terrain:** The original `.obj` mesh is rendered as a grey 3D surface.
* **Signal Strength Heatmaps:** 
  * The color of each point maps precisely to the dBm values calculated earlier (`power_bs_28.0` for the baseline, and `power_combined_28.0` for the RIS).
  * Points receiving less than `-190 dBm` (total shadow) are filtered out.
* **Hardware Generation:** Custom cylinder logic visually draws the 50m Starship and 100m Mast.
* **Interactive Toggles:** Plotly `updatemenus` are attached to the layout, injecting JavaScript to toggle the `visible` state of the Baseline point cloud vs the RIS point cloud.

---

## 4. Source Code (`sionna_malapert_sim.py`)
